...
def generate_tier1_cases(
    ...
    for name, inst in sorted(analysis.instructions.items()):
        ...
        out.emit(f"#if defined(Py_TAIL_CALL_INTERP)\n")
        out.emit(f"int predicted_opcode = next_instr->op.code;\n")
        out.emit(f"py_tail_call_funcptr next_op_f = INSTRUCTION_TABLE[predicted_opcode];\n")
        out.emit(f"(void)next_op_f;\n")
        out.emit(f"#endif\n")
